...
def get_raw_data():
    p_in = p_weibo_data+r'红海危机\红海危机.xlsx'
...
def split_words(row, stopwords, synwords):
    ...
    return sentence, seg_sentence(sentence, stopwords, synwords)
...
def main():
    ...
    jieba.load_userdict(p_lda_file+'自建词表.txt')#加载自建词表
    stopwords = stopwordslist(p_lda_file+'停用词表.txt')  # 这里加载停用词的路径
    ...
    for _, row in tqdm.tqdm(raw_data.iterrows()):
        if isinstance(row['微博正文'], str):
            sentence, words = split_words(row, stopwords, synwords)
            sentence_split.append(words)
    ...
    pd.to_pickle(sentence_list, p_out+'sentence.pkl')
    ...
    pyLDAvis.save_html(prepared_data, p_out+'ldavis.html')
