...
def main():
    ...
    target_df.columns
    # target_name = 'y_ret_wave_5m'
    target_name = 'y_ret_wave_1m'
    # target_name = 'y_ret_1m'
    target_df = target_df.fillna(0)
    target = target_df[target_name]
    target_benchmark = target_df['y_ret_1m']
    ...
    idx_train = factor.index[:int(len(factor)*train_pct)]
    idx_valid = factor.index[int(len(factor)*train_pct):int(len(factor)*(train_pct+valid_pct))]
    idx_test = factor.index[int(len(factor)*(train_pct+valid_pct)):]

    train_x0, valid_x0, test_x0 = factor.loc[idx_train], factor.loc[idx_valid], factor.loc[idx_test]
    ...
    factor_col_select = corr[corr.abs() >= 0.01].index
    ...
    for model_name in model_name_list:
        this_log.info('{} begin'.format(model_name))
    ...
    # evaluate_model(pv_lgbm, test_y, test_y_benchmark)
    # print(pd.DataFrame(eval_dict).T.to_string())
    eval_df = pd.DataFrame(eval_dict).T
    print(eval_df.to_string())
    eval_df.to_csv(p_out+'evaluation_minutely.csv')
...
def main_daily_model():
    ...
    target_benchmark = target_daily['close_to_close']
    # corr = factor.corrwith(target)
    # corr.abs().sort_values().dropna()
    # corr.abs().describe()
    # target.describe()
