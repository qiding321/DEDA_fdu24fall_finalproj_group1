...
def clean(line):
    ...
    # 读取文件，并按行保存为list
    # 
    deleteword = open('text test/2_LDA/weibo_deleteword.txt', 'r', encoding='utf-8').readlines()
...
def delete_stopwords(sourcedata, dir_path):
    ...
    for filename in filelist:
        ...
        if filename.endswith('.txt'):
            logger.info("正在读取停用词文件: " + filename)
            with open('./stopwords/' + filename, 'r', encoding='UTF-8') as f:
                for line in f:
                    stopword.append(line.strip())
        logger.info("开始去除...")
        logger.info("sourcedata: " + str(len(sourcedata)) + " | stopwords: " + str(len(stopword)))
...
if __name__ == '__main__':
    ...
    excel_name = 'text test/content data/红海危机'
    df = pd.read_excel(excel_name + '.xlsx')

    # df change to list
    data = df['微博内容'].values.tolist()
    # 删除标点符号和特殊字符
    logger.info("删除标点符号和特殊字符")
    ...
    # 去除停用词
    data = delete_stopwords(data, 'text test/2_LDA/停用词表.txt')
